export default [
  {
    icon: "/assets/icons/react-logo.png",
    name: "React",
    description: "as my general Javascript framework/library"
  },
  {
    icon: "/assets/icons/gatsby.png",
    name: "Gatsby",
    description: "as my frontend framework for non dynamic needs"
  },
  {
    icon: "/assets/icons/next.png",
    name: "Next",
    description: "as my frontend framework for dynamic scalable needs"
  },
  {
    icon: "/assets/icons/tailwind.png",
    name: "Tailwind",
    description: "as my CSS Framework"
  },
  {
    icon: "/assets/icons/figma.png",
    name: "Figma",
    description: "as my designing/wireframe tool"
  },
  {
    icon: "/assets/icons/meteor.png",
    name: "Meteor",
    description: "when I need to build minimal vaiable products"
  },
  {
    icon: "/assets/icons/node.png",
    name: "Node",
    description: "as my general backend JS runtime"
  },
  {
    icon: "/assets/icons/express.png",
    name: "Express",
    description: "for building APIs and webservers "
  },
  {
    icon: "/assets/icons/puppeteer.png",
    name: "Puppeteer",
    description: "for web automation and scraping data"
  },
  {
    icon: "/assets/icons/jest.png",
    name: "Jest",
    description: "for Test Driven Development / Testing"
  },
  {
    icon: "/assets/icons/cucumber.png",
    name: "Cucumber",
    description: "for Behavioural Driven Development / Testing"
  },
  {
    icon: "/assets/icons/google.png",
    name: "Google Analytics and SEO",
    description: "as my analytics platform"
  }
]